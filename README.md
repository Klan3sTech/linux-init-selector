...
2. При необходимости сначала установите альтернативную init-систему:
   ...
   На Debian/Ubuntu помощник намеренно **не** устанавливает пакеты-заменители системного init (`runit-init`, `openrc-init`, `sysvinit-core`): они могут конфликтовать с текущим пакетом, владеющим `/sbin/init`. Для выбора через init-selector достаточно обычного пакета `runit` и его бинарника; stage-скрипты и сервисы всё равно необходимо настроить отдельно.
